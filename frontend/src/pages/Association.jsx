...
const Association = () => {
  ...
  const fetchPost=async()=>{
    try{
      const res= await axios.get(URL+"/api/association")
    ...
    <div>
      <TopNav/>
        ...
        <div className="flex justify-between items-center">
         <h1 className="text-2xl font-bold text-black md:text-3xl">{post?.[0]?.title}</h1>
         {user && <div className="flex items-center justify-center space-x-2">
            <p className="cursor-pointer" onClick={()=>navigate("/association/"+post?.[0]?._id)} ><BiEdit/></p>
       ...
       <p>{new Date(post?.[0]?.updatedAt).toString().slice(0,15)}</p>
       <p>{new Date(post?.[0]?.updatedAt).toString().slice(16,24)}</p>
       </div>
        </div>
        <img src={IF+post?.[0]?.photo} className="w-full  mx-auto mt-8" alt=""/>
         <p className="mx-auto mt-8">{post?.[0]?.desc}</p>
